...
// query param names (date_from, location, item, ...).
//
...
// mirroring the dashboard app's old schema_v2.sql `sales` table. is_credit
...
credit_notes AS (
  ...
  -- is_credit from the CN's own DisplayTerm, same rule as cash/credit above.
  SELECT
    cn.DocNo AS doc_no,
...
// clean REST names (date_from, location, ...), not the old p_-prefixed ones.
export function salesCommonWhere(alias = 's') {
  return `
    ...
    AND (@location IS NULL OR ${alias}.location_id = @location)

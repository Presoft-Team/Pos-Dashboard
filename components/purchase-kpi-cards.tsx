import { PurchaseKpiSummary } from '@/types'
...
// purchase metric.
//
export default function PurchaseKpiCards({ data }: { data: PurchaseKpiSummary[] }) {
  const amount = (value: number | undefined) => Number(value ?? 0)
  const totalPurchase = formatMoneyLines(
    [...data].sort((a, b) => amount(b.total_purchase) - amount(a.total_purchase)).map((d) => ({ currency: d.currency, amount: amount(d.total_purchase) }))
  )
  const totalRevenue = formatMoneyLines(
    [...data].sort((a, b) => amount(b.total_revenue) - amount(a.total_revenue)).map((d) => ({ currency: d.currency, amount: amount(d.total_revenue) }))
  )

    ...
    <div className="bg-white border border-gray-200 rounded-lg shadow-lg p-3 text-sm">
      ...
      {row.breakdown.map((b, i) => (
        <p key={b.currency} style={{ color: CHART_PALETTE[i % CHART_PALETTE.length] }}>
          {b.currency}: {formatMoney(b.revenue, b.currency)} · Qty {fmtInt(b.qty)}
        </p>
      ))}
    </div>
  )
...
// revenue and quantity broken out separately in the tooltip.
export default function BarChartWidget({ data, color = '#F2AA24' }: Props) {
